...
interface Props {
  ...
  onClose: () => void;
  /** Opens the keyboard shortcuts cheat sheet and closes Settings. */
  onOpenCheatSheet?: () => void;
}

export default function SettingsDialog({ settings, onUpdate, onClose, onOpenCheatSheet }: Props) {
  ...
  return (
    ...
    >
      <div ref={trapRef} className="bg-stone-900 border border-stone-700 rounded-2xl shadow-2xl w-[440px] max-w-[90vw] max-h-[90vh] flex flex-col overflow-hidden">
        ...
        {/* Footer */}
        <div className="px-5 py-3.5 border-t border-stone-700 shrink-0 flex items-center gap-2">
          {/* Keyboard shortcuts link — opens the full cheat sheet */}
          {onOpenCheatSheet && (
            <button
              type="button"
              onClick={onOpenCheatSheet}
              className="flex items-center gap-1.5 text-[11px] text-stone-500 hover:text-stone-300 transition mr-auto rounded focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-brand-500/50"
            >
